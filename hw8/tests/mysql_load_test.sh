#!/bin/bash
# ============================================================
# MySQL Performance Test — 150 operations
# 50 create_cart + 50 add_items + 50 get_cart
# Output: results/mysql_test_results.json
# ============================================================

set -e

ALB_URL="${1:?Usage: ./mysql_load_test.sh <ALB_URL>}"
OUTPUT_FILE="../results/mysql_test_results.json"
CART_IDS=()
...
for i in $(seq 1 50); do
  ...
  RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$ALB_URL/mysql/shopping-carts" \
...
for i in $(seq 0 49); do
  ...
  RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$ALB_URL/mysql/shopping-carts/$CART_ID/items" \
    -H "Content-Type: application/json" \
...
for i in $(seq 0 49); do
  ...
  RESPONSE=$(curl -s -w "\n%{http_code}" "$ALB_URL/mysql/shopping-carts/$CART_ID")
